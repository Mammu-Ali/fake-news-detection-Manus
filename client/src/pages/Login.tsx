...
import { hasOAuthConfig, startLocalDemoLogin, startLogin } from "@/const";
...
export default function Login() {
  ...
  const handleLogin = () => {
    setLoginError("");
    setLoginStatus(hasOAuthConfig ? "OPENING_OAUTH_GATE..." : "OPENING_LOCAL_DEMO_NODE...");
    if (typeof window !== "undefined") window.history.replaceState({}, "", "/login");
    try {
      if (hasOAuthConfig) startLogin();
      else startLocalDemoLogin();
      window.setTimeout(() => {
        setLoginStatus("");
  ...
  return <main className="login-page">
    ...
    <section className="login-card" aria-labelledby="login-title">
      ...
      <button type="button" className="detect-button login-button" onClick={handleLogin} disabled={Boolean(loginStatus)}><LogIn size={16} /> {loginStatus ? "CONNECTING..." : hasOAuthConfig ? "USER LOGIN" : "LOCAL DEMO LOGIN"} <ArrowUpRight size={16} /></button>
      {loginStatus && <div className="login-status" role="status">{loginStatus}</div>}
      {loginError && <div className="login-error" role="alert"><strong>AUTH GATE ERROR</strong><span>{loginError}</span><small>{hasOAuthConfig ? "Check the OAuth callback URL and provider configuration." : "Local demo mode is active. Configure VITE_APP_ID and VITE_OAUTH_PORTAL_URL for real OAuth."}</small></div>}
      <div className="login-assurance"><ShieldCheck size={15} /><span>{hasOAuthConfig ? "OAuth session · protected dashboard · private history" : "Local demo session · protected dashboard · sample history"}</span></div>
